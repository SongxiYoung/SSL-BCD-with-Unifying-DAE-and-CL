...
parser.add_argument('--version', type=str, default='daevit_re', help='version of the experiment')
...
parser.add_argument('--pretrained_model', type=str, default="output_dir/daevit_re/model_best.pth.tar", help="path to pre-trained model")
...
parser.add_argument('--csv_eval', type=str, default='csvs_buffer/sub_valid_wo_unclassified_prepost.csv', help='train csv sub-path within data path')
...
def main(args):
    ...
    net = dae.__dict__[args.model](norm_pix_loss=args.norm_pix_loss).to(device=device)
...
def train_classifier(feature_label, n_samples_per_class=8000):
    ...
    # Take the first n_samples_per_class from each class
    sampled_feature_label = []
    for cls, fl in feature_label_by_class.items():
        sampled_feature_label.extend(fl[:n_samples_per_class])

    # Extract features and labels
    similarity = [x['similarity'] for x in sampled_feature_label]
    damage_label = [x['bldg_damage'] for x in sampled_feature_label]
...
def extract_features(net, data_loader, device):
    ...
    for i, batch in enumerate(data_loader):
        ...
        loss, recon_batch = net(bldg_post)
