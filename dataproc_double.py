...
class xBD_Building_Polygon_TwoSides_PrePost(Dataset):
    ...
    def __init__(self, data_path, csv_file, transform=None, eval_mode=False):
        """
        Args:
    ...
    def resample(self, num_samples_per_class=8000):
        # Calculate the number of samples for each class
        class_counts = self.df['bldg_damage_code'].value_counts()

        # Create a list of indices for each class
        class_indices = [self.df[self.df['bldg_damage_code'] == i].index.to_numpy()
                         for i in class_counts.index]

        # Resample indices for each class
        resampled_indices = [np.random.choice(indices, num_samples_per_class, replace=True)
    ...
    def __getitem__(self, idx):
        if self.eval_mode:
            idx = self.resampled_indices[idx]
...
class xBD_Building_Mask_TwoSides_PrePost(Dataset):

    def __init__(self, data_path, csv_file, transform=None):
        """
        Args:
            data_path (string): directory with all images and masks
            csv_file (string): csv file with image id's
            transform (callable, optional): optional transform to be applied to a sample image.
        """
        super(xBD_Building_Mask_TwoSides_PrePost, self).__init__()
        self.data_path = data_path
        self.csv_file = csv_file
        self.df = pd.read_csv(os.path.join(data_path, csv_file))
        self.transform = transform
    ...
    def __repr__(self):
        return "Dataset, directory: {}\ncsv: {}\ntransform: {}\nclass_weights: {}".format(
            self.data_path, self.csv_file, self.transform, self.class_weights
        )
